...
- Production pricing is now final. `/pricing` may emit Essential and Practice `Offer` markup in AUD. Do not emit `valueAddedTaxIncluded: true`. River Aftercare is not registering for GST at this stage. Do not emit a numeric Group offer or an internal Group floor.
