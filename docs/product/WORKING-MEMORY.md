...
Last updated: 2026-09-20 (pricing cards: Essential can create/edit up to 2 custom guides; Practice differentiates by allowance and template adaptation)
...
| Verify      | `pnpm observability:test-error` on a trusted machine. Loads ignored `.env` if DSN is unset in the shell. Does not require `VERCEL_ENV=production`. Does not enable local app telemetry. Event: `river_aftercare_error_tracking_verification`, `environment=verification`. Do not run from CI. The standalone `.mjs` CLI must use `@sentry/nextjs`'s CJS `default` export — Node ESM named imports do not include `captureEvent`/`flush`. |
