...
Last updated: 2026-09-20 (account settings, operator clinic support, STAFF Active/Inactive)
...
River Aftercare is **not GST registered**. GST registration is pending accountant confirmation. Stripe GST/tax configuration is **not authorised**.
...
| Production prices | Public: Essential **A$79/month or A$790/year**, Practice **A$149/month or A$1,490/year** (Recommended), Group **Custom pricing**. Public prices are advertised AUD amounts. Do not claim GST is included: GST registration is pending accountant confirmation, current legal status is not GST registered, and Stripe GST/tax configuration is not authorised. Additional-location **rates are internal only** (2nd A$79/A$790, 3rd+ A$59/A$590) until a Location model exists — public copy is “talk to us about your setup”; do not advertise a larger guide library. Canonical source: `lib/marketing/plans.ts` (card features + `PLAN_COMPARISON_ROWS`). **Concise cards:** Essential = 1 practice/location, River Aftercare guide templates, create and edit up to 2 custom clinic guides, up to 2 clinic team members, clinic branding and curated typography\*, QR sharing / PDF / durable patient guide URLs. Practice = Everything in Essential, up to 30 custom clinic guides, adapt River templates, up to 5 clinic team members, Assisted setup, Priority support, plus the multi-location talk-to-us callout. Group stays the current four truthful bullets (including Priority support; do not invent a higher Group support tier). Comparison **Support** row uses text values, not checkmarks: Essential = Standard support, Practice = Priority support, Group = Priority support. Do not put Standard support on the Essential card. Do not publish response-time SLAs. Do not say “permanent guide URLs”; public wording is **durable patient guide URLs**. Clinic team member allowances (Essential 2 / Practice 5; ADMIN and STAFF count, OPERATOR does not) are public copy only — no per-seat billing and no enforcement yet. Use “clinic team members”, never “seats”. Guide counts and template-adaptation entitlement remain commercial copy, not technically enforced. Essential may use River templates as supplied and fully edit clinic-owned guides; it cannot commercially adapt River templates. A collapsed **Compare all plan features** disclosure under the typography footnote expands a comparison table (desktop) / stacked groups (below 64rem). Do not use per-card Read more, pricing modals, extra-seat prices, or numeric location prices. Typography footnote unchanged: six curated typefaces, additional options on request — do not list font names on `/pricing`. |
...
| Seats (policy) | Public pricing now states **up to 2 / up to 5 clinic team members** (Essential / Practice). Group remains tailored. ADMIN and STAFF count; River OPERATOR does not. No per-seat billing and no enforcement yet. Do not say “seats” in public copy. |
...
| Area                       | Behaviour                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Reveal grammar             | Existing Motion system only: `MarketingRevealGroup` / `Item` / `Card`, `editorialRevealDelay` / `cardRevealDelay`, `mkReveal`, `data-mk-pending`, `useReducedMotion`. Do not add a second animation system.                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Privacy     | `sendDefaultPii: false`, `includeServerName: false`, `tracesSampleRate: 0`, `maxBreadcrumbs: 0`, no local variables, no attachments, no Prisma/OTEL tracing, no module inventory, no device/OS/culture contexts. `beforeSend` strips user/headers/cookies/query/body/`server_name`. Verification uses this same floor.                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Verify      | `pnpm observability:test-error` on a trusted machine. Shares `lib/observability/error-tracking-privacy.ts` with production (`beforeSend`, integration denylist, `sendDefaultPii: false`, `includeServerName: false`). Loads ignored `.env` if DSN is unset in the shell. Does not require `VERCEL_ENV=production`. Does not enable local app telemetry. Event: `river_aftercare_error_tracking_verification`, `environment=verification`. Do not run from CI. The standalone `.mjs` CLI must use `@sentry/nextjs`'s CJS `default` export — Node ESM named imports do not include `captureEvent`/`flush`. SDK payload has no `user`, `server_name`, or `modules`; Better Stack may still enrich IP from the ingest connection. |
...
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Essential | 1 practice / location; River Aftercare guide templates; **Create and edit up to 2 custom clinic guides**; branded pages; logo/colours/typography*; permanent URLs; QR-ready; Print / Save PDF; contact/emergency; Light/Dark/System |
| Practice  | Everything in Essential; **Up to 30 custom clinic guides**; **Adapt River Aftercare templates to suit your clinic**; Assisted setup. Supporting callout remains “Multi-location practice? Talk to us about your setup.”             |
...
Authenticated staff/operator work only. No marketing, branding, or pricing changes. Additive Prisma migration `20260920213000_add_clinic_membership_active` (`ClinicMembership.active BOOLEAN NOT NULL DEFAULT true`). Do not run on production from this branch.
...
| Email verification | None exists. Do not invent a mail token flow. Immediate email update, clear `emailVerified`, revoke outstanding `PASSWORD_RESET`. Session stays (user id).                                                                                                                                                                                                            |
| Operator support   | Platform `OPERATOR` is not a clinic member. `lib/auth/clinic-authorization.ts` grants clinic-owned manage permission to operators. **Manage clinic workspace** sets HttpOnly `river_operator_support_clinic`. Synthesized ADMIN-equivalent context `source: operator_support`. Operators cannot set another user's password or edit another user's global name/email. |
| Membership status  | Clinic ADMIN (and authorised operator) can Activate / Deactivate **STAFF** in that clinic only. Inactive memberships do not authorize that clinic. Other-clinic active memberships remain. Reactivation reuses the same row.                                                                                                                                          |
