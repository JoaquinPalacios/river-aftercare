...
- Billing actions keep the current page visible while they run. Schedule downgrade, Cancel plan change, Keep Practice, Review downgrade, guide-selection save, and Manage billing show an inline spinner and an action-specific pending label, and disable the conflicting control until the action returns. A failure restores the controls and uses the existing notice. Adopt the same `PendingSubmitButton` on the remaining mutation forms later; that sweep is recorded in `docs/product/POST-LAUNCH-ROADMAP.md`.
