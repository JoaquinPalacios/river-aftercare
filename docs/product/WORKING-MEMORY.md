...
- Billing actions keep the current page visible while they run. Schedule downgrade, Cancel plan change, Keep Practice, Review downgrade, guide-selection save, and Manage billing show an inline spinner and an action-specific pending label, and disable the conflicting control until the action returns. The spinner is mounted only while that action is pending, and an idle button contains its label with no spinner placeholder. During an unscheduled review, Cancel plan change is at the top of Change plan and again beside Schedule downgrade; both submit the same cancellation and share its pending state. A successful cancellation returns to `/account/billing` and the Review downgrade entry. A failure restores the controls and uses the existing notice. Adopt the same `PendingSubmitButton` on the remaining mutation forms later; that sweep is recorded in `docs/product/POST-LAUNCH-ROADMAP.md`.
