...
- The pre-payment gate blocks product authoring only when entitlement is `PENDING`. Clinics with no entitlement row stay on today's access. `ACTIVE` opens the product. `RESTRICTED` and `ENDED` are not locked yet.
...
| Stripe Billing Phase 2    | LOCAL — DEMO-APPROVED CHECKOUT. PRE-PAYMENT GATE FOR PREPARED CLINICS ONLY. PORTAL AND PLAN LIMITS NOT BUILT                              |
...
| Enablement  | `VERCEL_ENV` `production` or `preview` + valid `NEXT_PUBLIC_SENTRY_DSN` (optional server `SENTRY_DSN`). Dev/test/local off. Preview is labelled `preview`.                                                                                                                                                                                                                                   |
...
| Gate       | No `ClinicEntitlement` → legacy, product stays open. `PENDING` → billing setup or payment-processing only. `ACTIVE` → product opens. `RESTRICTED` / `ENDED` stay unenforced (Phase 3 hook). Operator support access is not gated.                                                                               |
