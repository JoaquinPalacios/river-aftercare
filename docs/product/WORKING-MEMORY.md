...
- Billing actions keep the current page visible while they run. Schedule downgrade, Cancel plan change, Keep Practice, Change plan, guide-selection save, and Manage billing show an inline spinner and an action-specific pending label, and disable the conflicting control until the action returns. The spinner is mounted only while that action is pending, and an idle button contains its label with no spinner placeholder. During an unscheduled review, one plan-change action area states the next step and holds the current primary action with Cancel plan change: Confirm guide selection or Update guide selection while the keep-set is open, then Schedule downgrade once it is confirmed. Edit guide selection is a content-width secondary control in that area. Schedule downgrade stays a separate explicit action. When every clinic-owned guide already fits Essential, that same area still shows Schedule downgrade and Cancel plan change. No keep-set was stored, so Cancel plan change only leaves the review and does not call Stripe. On desktop the area is one compact row: a short next-step title, the primary action first, and Cancel plan change as a secondary button. It stacks on narrow screens. The dates and “no immediate charge” stay in the review below, not repeated in the sticky row. A successful cancellation returns to `/account/billing` and the Change plan entry. A failure restores the controls and uses the existing notice. Adopt the same `PendingSubmitButton` on the remaining mutation forms later; that sweep is recorded in `docs/product/POST-LAUNCH-ROADMAP.md`.
