...
Last updated: 2026-09-26 (Group → Practice billing checkpoint, audit only: `GROUP` is an entitlement label. No application path writes it, and the Stripe catalogue has no Group price. Commercial completion is not implemented. See the checkpoint section below. Earlier the same day: account split execution moves one prepared Site inside one PostgreSQL transaction: structure locks, readiness recomputation, guide/revision copies, placement id continuity, primary promotion, deactivation, compatibility slug parking, profile mirror, membership move, and session revocation. It does not call Stripe, Resend, or R2, and it does not convert Group to Practice. Multi-account login is still absent. See [../architecture/ACCOUNT-SPLIT.md](../architecture/ACCOUNT-SPLIT.md). Account structure lock `clinic-account-structure:{clinicId}` serializes site, location, guide, placement, membership, entitlement, and billing-projection writes inside the mutation transaction. An open preparation does not freeze the Account. Sequential splits set destination `Clinic.slug` from the moved Site and source `Clinic.slug` from the actual post-execution primary, not blindly from the kept Site. Production font builds load vendored latin WOFF2 files through `next/font/local`. Account split preparation: one SPLIT Site per preparation, `RETAIN_ON_SOURCE` for later sequential splits, split readiness separate from Practice downgrade readiness, kept Site need not be primary, destination shell uses the existing invite / legal / Checkout / `invoice.paid` path with zero Sites. No site move, guide copy, membership move, primary mutation, or Group → Practice billing.) Production marketing static delivery verified on `97ce087`; homepage median TTFB 84.8ms and pricing 55.0ms from a US runner; patient and staff hosts stay `no-store`. Sentry client bundle: Turbopack `compiler.define` drops unused debug/tracing code from the shared browser chunk, 371,081 → 303,628 bytes uncompressed; early `instrumentation-client` init and server Sentry stay; production transfer of the previous chunk was about 120 KB brotli, immutable, CDN HIT)
...
`ClinicSite` is the public clinic identity. It owns the hostname and branding (display name, logos, favicon, colours, dark branding, neutral, radius, typeface, terminology, theme, patient theme toggle, attribution). `ClinicSite.slug` is immutable in normal clinic and operator UI. `isPrimary` does not transfer a hostname. There is no primary-site-changing UI.

`ClinicLocation` belongs to a `ClinicSite`. The root location (`servesSiteRoot`, `slug` null) keeps the unprefixed patient URLs. Additional locations use `https://{siteSlug}.{root}/{locationSlug}` and `/{locationSlug}/{guideSlug}`, including `/print`. Patient brand headings use `ClinicSite.displayName`. Location display name is the place, not the brand. Locations inherit the site brand. There is no per-location logo. Inactive sites and locations 404. Patient pages stay `noindex` and are not in the sitemap. `proxy.ts` stays database-free. The first URL segment prefers an enabled root guide over a location. A disabled root placement does not block creating a location with that slug. Publishing or re-enabling a root guide whose slug matches any non-root location slug (active or inactive) fails with `ClinicPortalError` and rolls back, so a location URL cannot be silently shadowed. Creating or changing a root guide slug onto an existing location slug fails the same way. Enforcement is `assertLocationSlugAvailable` and `assertRootGuideSlugAvailable` in `lib/clinics/slug-collisions.ts`, called from location create and from root placement upsert, slug align, and root re-enable. There is no schema exclusion constraint.

Staff navigation: Practice settings stay at `/practice`. Sites & Locations is `/practice/sites` and `/practice/sites/[siteId]`. Clinic admins manage sites, branding, and locations. Staff can view them and manage placements where guide editing already allows it. Site deactivation is operator-only. Root locations are not deactivated on their own. There is no hard delete.

`PracticeGuide` stays account-owned and counts once against the guide allowance. `PracticeGuidePlacement` publishes that guide at one location. Publish still advances only the primary root placement. Other placements stay pinned. “Use latest version” moves one placement onto the latest clinic `PracticeGuideRevision`. A null pin keeps the canonical template fallback. There is no per-placement template revision column. “Create location-specific copy” creates a new account guide (`copiedFromPracticeGuideId`), copies the visible content, and moves only that placement. Global unpublish disables every placement for the guide. Removing a guide from one location disables only that placement.

`ClinicProfile` stays for the original primary site and root location. New sites and additional locations do not dual-write it. Branding object keys stay `clinics/{clinicId}/branding/{uuid}.{ext}`. A later optional layout is `clinics/{clinicId}/sites/{siteId}/branding/...`. Do not migrate R2 objects.

Capacity is enforced in `lib/clinics/site-location-capacity.ts` with the account advisory lock `clinic-site-location-capacity:${clinicId}`. Count active sites, and locations that are active on an active site. Essential is always 1/1. Practice is always 1 site, with `locationAllowance` (minimum 1). Group uses the stored allowances. A missing entitlement is 1/1, never unlimited. Creating a site consumes one site and one location. Operator sets stored allowances. That does not charge or refund. Stripe add-on billing is not implemented.
...
## Group → Practice billing checkpoint

Audit only, 2026-09-26. Do not change Stripe, Prisma, plans, or execute a split from this note. Do not encode the undecided Group prices below as Stripe Prices.

`CommercialPlan.GROUP` is a Prisma enum value and an ungoverned entitlement label. `prepareClinicCommercialOffer` and the operator billing form accept only Essential and Practice. `executeClinicCheckout` returns `group_unavailable`. `assessOperatorPlanUpgrade` returns `group_unavailable`. `assessClinicPlanDowngrade` returns `unsupported`. `projectEntitlement` maps only the four Essential/Practice env Price IDs and returns `unknown_price` for anything else, which the webhook marks failed and does not write. Public pricing is “Custom pricing” / Talk to us. `STRIPE_CATALOG_SLOTS` has no Group slot. `docs/launch/STRIPE-SETUP.md` says not to create a Group Product.

Nothing in application code sets `commercialPlan` to `GROUP`. Tests, e2e, and `scripts/audit/post-multilocation-query-baseline.test.ts` write it directly. A production Group row was not counted: `DATABASE_URL` was unset here, Prisma Local discovery failed, and Prisma Remote was left unauthenticated. Do not invent a count. The states the app can represent are whatever a direct write stored: the enum, stored `siteAllowance` / `locationAllowance`, optional extras, and optional Stripe ids. Product behaviour when the row is Group: guide and team caps are not applied (`planGovernanceFromEntitlement` reason `group`); site and location caps are the stored integers; operator extras cannot be edited; staff product access follows `entitlementStatus` (`ACTIVE` stays open); published patient guides stay public until entitlement is `ENDED`.

Advertised self-serve amounts, from `lib/marketing/plans.ts` and the Stripe setup note: Essential A$79/month and A$790/year; Practice A$149/month and A$1,490/year. Checkout always sends quantity 1 of that base Price. Practice second-location A$79/A$790 and later-location A$59/A$590 are internal `PLAN_PRICES` references, not Stripe Prices, and annual add-on prices are not an approved charge. `stripePriceId` is an unconstrained string on `ClinicEntitlement`, not on `ClinicBillingProfile`. The profile stores customer, subscription, schedule, Practice→Essential attempt, and Checkout session ids. Projection will not persist an unknown Price.

`practiceDowngradeReady` is recalculated on read by `readPracticeDowngradeReady` / `assessExecutedPracticeDowngrade`. It is not a stored commercial flag. Practice base plus the source extras is the team and guide ceiling. Guides with no placements remain and still count. Over that ceiling, more than one active Site, more active Locations than the stored location allowance, or a copied guide left with no placement rows keeps it false. Execution does not change the source plan, extras, allowances, or Stripe, and it does not delete guides. A later open preparation hides the completed card. Commercial completion must rerun that current-state check after the latest `COMPLETED` preparation. An intermediate split must not bill. A deactivation-only path with no destination is not supported: preparation requires exactly one `SPLIT` Site. Leave that as a later product case.

The destination shell’s offer → customer Checkout → `invoice.paid` path is not safe to reuse on an existing source Group. Checkout refuses `entitlementStatus` `ACTIVE`, a paid billing status, `stripeSubscriptionId`, and `GROUP`. The offer writer refuses an active entitlement or any subscription id, and a successful offer sets `PENDING`, which closes staff product routes. Those guards stop a second Checkout. They do not replace a subscription. Stripe Checkout `mode: subscription` would create another subscription if a Customer already had one and the local subscription id was missing. The Essential→Practice item update also rejects Group, and it requires the live Price to already be the Essential catalogue Price, one item, quantity 1. An unmapped Group Price fails `price_mismatch`. Customer Portal plan switching is refused. `checkout.session.completed` does not activate. `invoice.paid` with a mapped Practice Price is the event that may set local `commercialPlan` to Practice. That update does not clear extras or site/location allowances. Practice enforcement then treats the site cap as 1 and keeps the stored location allowance, which is operator capacity, not a Stripe quantity. Inheriting a Group location allowance of 5 onto Practice would allow unpaid extra Locations. Charging those Locations is not possible with the current catalogue.

Legal acceptance for a new Checkout is `LegalAcceptance` source `BILLING_CHECKOUT`, Terms `2026-09-21`, Privacy `2026-09-21`, for that clinic and that user. Current versions already accepted satisfy it. The operator upgrade path records no new acceptance and is not valid for Group. The customer must accept a new price, interval, and missing legal versions. The operator may prepare. The operator must not start a new paid subscription. Immediate versus period-end timing is not decided: Essential→Practice is immediate `always_invoice`; Practice→Essential is a period-end schedule; Group has neither. Do not invent that policy.

Failure stays commercial. The completed split is not rolled back. The source remains Group until a mapped `invoice.paid` projects Practice. Duplicate `invoice.paid` is receipt-idempotent. An unknown Price, a delayed webhook, Checkout expiry, or a failed card leaves Group. No first-release email is required; Stripe already sends invoices. R2 keys stay as the structural split left them.

PR shape when implementation is approved: C1 is recalculated readiness and separate structural versus commercial status, with no Stripe write. C2 is the Stripe change and webhook finalization. Do not reuse `stripePlanDowngradeAttemptId` (that is Practice→Essential). A new Checkout lifecycle can reuse `stripeCheckoutSessionId`. A one-item catalogue upgrade can reuse `river-plan-upgrade-{clinicId}-{subscriptionId}-{targetPriceId}`, but that key does not fit an unmapped Group Price without a new mutation.

Joaquín still has to decide: whether any live Group account has a Stripe subscription; whether a source with more than one Location may become Practice on the base Price while extra Locations stay an operator allowance; and whether a future Stripe change is immediate or at period end. Until then, do not implement PR C.

Next technical cleanup after that commercial series, not during it: load the six clinic font definitions only on patient guide surfaces and the guide editor/preview, and remove that catalogue from the general tenant layout if the layout does not use it.

Account split execution is operator-only and does not perform the Group to Practice downgrade. One open preparation per source Group Account names exactly one `SPLIT` Site, plus `DEACTIVATE` or `RETAIN_ON_SOURCE` for the others, and one destination shell. `executeClinicAccountSplit` reloads that preparation under source and destination `clinic-account-structure` locks and performs the cutover in one transaction. The kept Site is the eventual survivor, not necessarily the source primary after this split. Retained Sites stay active. Deactivated Sites stay as rows and stop resolving. Moving placements keep their ids. Guide copies set `copiedFromPracticeGuideId` null and record guide and revision maps. Public slugs are unchanged. R2 objects are not copied. Destination-only staff leave the source; their sessions are deleted. `COMPLETED` is the last write. A retry returns that result. `practiceDowngradeReady` is reported afterward and does not change the source plan. An open preparation does not freeze the Account. Multi-account login and the Stripe Group to Practice conversion are still not implemented. Detail: [../architecture/ACCOUNT-SPLIT.md](../architecture/ACCOUNT-SPLIT.md).
...
Production verification, 2026-09-25, `main` `97ce087` (PR #105), from a US runner whose edge was `iad1`: homepage median TTFB 84.8ms (before 301.3ms) and pricing 55.0ms (before 311.8ms). Warm marketing responses were `x-vercel-cache: HIT` or, on the first pricing and contact requests, `PRERENDER`, with `x-nextjs-prerender: 1` and an `x-vercel-id` of `iad1::<id>` (no `syd1` function segment). `demodental` and `app` `/login` stayed `private, no-store`, `MISS`, and `iad1::syd1::<id>`. The contact form and Turnstile client chunk were present; the form was not submitted. Operator SEO was not edited in production. Do not open patient query work next only because statement counts are high. The 420-byte demo logo now caches after the first miss (`HIT`, `age` 21). Detail is in [Marketing Static Delivery — Production Verification](../performance/POST-MULTILOCATION-PERFORMANCE-AUDIT.md#marketing-static-delivery--production-verification).

The Function region pin is in root `vercel.json` (`"regions": ["syd1"]` only). Production now executes Functions in `syd1`. Region mismatch: **RESOLVED**. Measured 2026-09-25 from a US runner (`x-vercel-id` `iad1::syd1::<id>`). `demodental` home/guide/print medians fell from 1,304 / 1,513 / 1,504ms to 296.6 / 331.0 / 340.4ms. Warm `/api/health` stayed about 267–334ms, so the remaining patient gap above one database check is tens of milliseconds from this vantage. Do not open a patient query pull request next only because statement counts are large. The staff Zod / Prisma enum bundle split is measured in the audit’s [Staff client bundle split](../performance/POST-MULTILOCATION-PERFORMANCE-AUDIT.md#staff-client-bundle-split). Login first-load JavaScript fell from 1,082,709 bytes to 695,120 (−35.8%). Practice settings fell from 1,197,455 to 753,077 (−37.1%). The 387,769-byte Zod chunk and the 57,465-byte Prisma field chunk are absent from the production client graph. Patient home, guide, and print each changed by −2 bytes of shared-chunk noise. Server Zod schemas stay authoritative. The shared Sentry browser chunk was about 371KB uncompressed (about 120KB brotli on the production CDN, immutable). Turbopack `compiler.define` removed unused debug and tracing code; the local chunk is 303,628 bytes. Early client init stays. Detail is in the audit’s [Sentry Client Bundle Investigation](../performance/POST-MULTILOCATION-PERFORMANCE-AUDIT.md#sentry-client-bundle-investigation).

Before the Sydney pin, measured 2026-09-25 from a US runner whose Vercel id was `iad1`: public patient HTML is `private, no-store` and uncached (`x-vercel-cache: MISS`). `demodental` home/guide/print TTFB was about 1.3–1.5s. The same shapes on local PostgreSQL 18 were 20–70ms. Local EXPLAIN was sub-millisecond sequential scans because the fixture tables are tiny; the unique indexes already exist. The cost is round trips, not a missing index at this size.
...
Auth.js `Session` (`lib/auth/session.ts`, `auth_sessions`) is unrelated and stays.
...
Production application exceptions are sent to **Sentry** through `@sentry/nextjs` `10.75.0`. Do not upgrade to v11 in a bundle-size change; that is a separate migration. Telemetry is on only when `VERCEL_ENV` (browser: `NEXT_PUBLIC_VERCEL_ENV`) is `production` or `preview` and a valid DSN is present (`NEXT_PUBLIC_SENTRY_DSN`, optional server `SENTRY_DSN`). Browser bootstrap reads those `NEXT_PUBLIC_*` values as direct `process.env.NEXT_PUBLIC_*` member expressions so Next.js inlines them; do not read the `process.env` object or a dynamic key. `instrumentation-client.ts` calls `initClientErrorTracking()` synchronously before hydration. Do not dynamic-import that init: Next.js does not await async work there, so hydration errors would be missed. An absent release is omitted so it does not replace the release injected at build time. Production and preview stay distinguishable. Development, test, and local stay off. Session Replay is off. Tracing sample rate is 0. `BrowserTracing` is filtered. `next.config.ts` `compiler.define` sets `__SENTRY_DEBUG__` and `__SENTRY_TRACING__` false because Turbopack does not apply Sentry's webpack treeshake. Server `@sentry/nextjs` stays in `serverExternalPackages` and is not rewritten by those defines. URL fragments (including raw reset and invitation tokens) are stripped. Authorization/Cookie headers, passwords, and request bodies from login/reset/invitation/password-change are not sent. Staff email/name are not attached by River Aftercare. Patient PII is not captured. `/api/health` failures are not reported as error events. The app works if Sentry is unset or unavailable. Alerting is not active until Joaquín configures the Sentry project and Vercel env. Source-map upload is optional and must not fail the build.
...
Phase 5 **is implemented, merged, production-migrated, and manually accepted**. Later UX polish does not change billing, Stripe, entitlement, or retention semantics:
...
- Local `commercialPlan` stays Practice until a trusted webhook sees the Essential Price. `scheduledCommercialPlan`, `scheduledPlanEffectiveAt`, and `stripeSubscriptionScheduleId` are the Stripe schedule projection. They are not the guide-selection state. `stripePlanDowngradeAttemptId` is one scheduling lifecycle. Idempotency keys include it, so a new Schedule downgrade after Keep Practice does not replay the released schedule. River persists a scheduled downgrade only after a fresh retrieve shows the schedule is active and `subscription.schedule` is that id. The raw `from_subscription` schedule is one Practice phase with empty metadata and `proration_behavior: create_prorations`; a retry updates that same schedule instead of creating another. Final verification still requires `proration_behavior: none` on both phases. A scheduling failure leaves a confirmed keep-set in place. Cancel plan change reads the live subscription before it deletes that preparation. With no attached schedule it retires the attempt and deletes the preparation locally. An attached schedule that classifies as this attempt, including the raw one-phase `from_subscription` copy, is released and verified before that cleanup. A failed release or an unclassified schedule leaves the preparation, keep-set, and attempt in place. Keep Practice uses the same release verification for a fully scheduled downgrade. A cancellation that supersedes a validated downgrade also deletes the preparation. None of those paths retain guides.
- Team usage above the effective Essential team allowance still blocks scheduling. The administrator resolves it from Practice → Members, and Billing recomputes readiness on return. Guide overage does not block scheduling once the clinic ADMIN has confirmed a keep-set that fits Essential. The billing entry CTA is **Change plan**. It only opens the review. It does not schedule anything. Change plan creates that preparation when guides do not fit. If they already fit, River does not invent an empty keep-set. STAFF cannot confirm the selection. On `/account/billing`, an unchecked guide cannot be selected once that choice would pass the Essential custom, editable, or combined allowance. Already selected guides stay enabled. After confirmation the page shows the saved keep-set and the guides that will be retained for 60 days once Essential begins; Edit guide selection reopens the checkboxes from that saved set. The Billing page uses the same `max-w-5xl` staff width as Guides. Ordinary billing text stays in a `max-w-xl` reading column. A persisted preparation resumes after refresh. `?change-plan=essential` resumes an unpersisted review when guides already fit.
- At the Essential price transition, guides outside the keep-set are retained for 60 days from when Essential begins. A scheduled Practice → Essential boundary is that time. If it was not stored, the incoming Essential period start is used. Stripe `event.created` is only the last resort, because a Test Clock event is created on the real timeline before the simulated boundary. They stay readable at an existing published URL until then, stay out of active capacity and public listings, and are read-only until restored. The retained-guide card shows Retained, the publication state, and Read-only, plus the retention date. A retained **published** guide shows its live patient URL (the existing canonical patient URL) and says that page remains available until the retention date unless the guide is restored, after which it is no longer accessible. A retained **draft or unpublished** guide stays non-public: the card does not show a patient URL or call the guide live. Restore still requires a free place on the current plan, including any operator-granted allowance. The clinic-facing limit message tells the administrator to free a place or increase the clinic’s allowance. Keep Practice and a cancellation that replaces the downgrade clear the preparation and do not retain guides. There is no automatic purge.
- Billing actions keep the current page visible while they run. Confirm downgrade, Cancel plan change, Keep Practice, Change plan, guide-selection save, and Manage billing show an inline spinner and an action-specific pending label, and disable the conflicting control until the action returns. The spinner is mounted only while that action is pending, and an idle button contains its label with no spinner placeholder. During an unscheduled review, one plan-change action area states the next step and holds the current primary action with Cancel plan change: Confirm guide selection or Update guide selection while the keep-set is open, then Confirm downgrade once it is confirmed. The next-step title is “Next: confirm the downgrade”, and the pending label is “Confirming…”. Edit guide selection is a content-width secondary control in that area. Confirm downgrade stays a separate explicit action and still schedules the Practice → Essential transition. When every clinic-owned guide already fits Essential, that same area still shows Confirm downgrade and Cancel plan change. No keep-set was stored, so Cancel plan change only leaves the review and does not call Stripe. On desktop the area is one compact row: a short next-step title, the primary action first, and Cancel plan change as a secondary button. It stacks on narrow screens. The dates and “no immediate charge” stay in the review below, not repeated in the sticky row. A successful cancellation returns to `/account/billing` and the Change plan entry. A failure restores the controls and uses the existing notice. Adopt the same `PendingSubmitButton` on the remaining mutation forms later; that sweep is recorded in `docs/product/POST-LAUNCH-ROADMAP.md`.
- Plan-change confirmations are one latest action. Scheduling, Keep Practice, and cancelling a preparation do not leave older success messages on screen. Those confirmations, and other transient action notices that use `TransientNotice`, can be dismissed locally. Dismissing one does not change the scheduled plan, Stripe, or the persistent scheduled status. Successful scheduling logs `plan_downgrade_requested` and `plan_downgrade_scheduled` with the clinic, the acting user, Practice, Essential, and the billing interval. Keep Practice, and cancelling a fully configured schedule, log `plan_downgrade_reversed` with the same actor. Cancelling a local or unattached preparation logs `plan_downgrade_preparation_cancelled`. Releasing a partial attached schedule logs `plan_downgrade_partial_schedule_released`. A failed cancellation logs `plan_downgrade_cancel_failed`.
- The guide editor keeps the Published/Draft lifecycle pill as the durable state. A draft save shows Saving… while pending, then Saved for 2.5 seconds (`SAVED_CONFIRMATION_MS`), then hides that confirmation and its success sentence. Another successful save restarts the 2.5 seconds. Save and publish errors stay on screen. Practice settings still shows a persistent Saved state. The editor status line reserves a small minimum height so hiding Saved does not shift the header. The editor toolbar is sticky on desktop and mobile. Its stuck inset is `--staff-sticky-inset` (`0.75rem`, 12px) below `env(safe-area-inset-top)`. That padding is taken from the resting margin, so the unscrolled header does not move, and the canvas background covers the inset.
- Additive migrations `20260923200000_add_scheduled_plan_downgrade`, `20260923220000_add_downgrade_guide_selection`, and `20260924010000_add_plan_downgrade_attempt` were applied in production with Phase 5. The UX polish does not add a migration. The Subscription Schedule phase shape is unchanged from the Sandbox acceptance. Idempotency keys now include the attempt id.

Phase 5 manual acceptance, including the Practice → Essential downgrade, has passed. See `docs/launch/STRIPE-SETUP.md` for the Sandbox procedure. UX polish after that acceptance does not change Stripe schedule behaviour.
...
| Staff host         | `app.localhost` continues to serve `/`, `/login`, and the clinic dashboard. Retired chairside URLs 404. |
...
Marketing motion/navigation polish, calmer patient interactions, and the first Aftercare Guide clinic portal. No Check-ins. No RecoveryPlan. No fake analytics. At this phase, chairside remained parked and reachable by direct URL. It was later removed; see Durable legacy chairside removal.
...
Staff `/dashboard` is the River Aftercare clinic portal.
...
| Chairside | Removed. `/dashboard/procedures`, `/sessions/new`, `/session/[id]/control`, and `/display/[token]` 404. Not linked from portal nav.                                                                                                                                                                                                                                                                                                     |
...
| Area         | Behaviour                                                                                                                                                                                                                                                                                                                                                         |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Provider     | `CLINIC_ASSET_STORAGE_DRIVER=r2` + `@aws-sdk/client-s3` server-only. Supabase Storage clinic-asset adapter removed. Chairside Supabase Realtime was later removed with the legacy live-session feature.                                                                                                                                                           |
...
| Group plan      | Public copy stays commercial/onboarding (“coordinated rollout”). It does not advertise site or location rates or a self-serve Group checkout. The staff product can manage sites and locations; that is not a marketing claim.                                      |
...
| GST            | Investigation recommendation only — **not current public/legal truth**. Public GST-inclusive claims have been withdrawn. Current decision: River Aftercare is not registering for GST. Do not charge GST, implement Stripe Tax, or label prices as including or excluding GST. Do not implement inclusive 10% GST from this table. |
...
Typeface architecture: six allow-listed families in `lib/branding/clinic-fonts.ts`, loaded with `next/font/local` from `lib/branding/font-files` (latin WOFF2, `preload: false`). Geist and Geist Mono use the same local files. The tenant layout selects one family at request time. Next.js still emits every statically reachable `@font-face` into the tenant CSS graph, so per-family dynamic import still `<link>`s the whole catalogue and adds stylesheet requests. Font _files_ are fetched only for the applied family (Geist default: 1 WOFF2; Inter selected: 1 Inter WOFF2). Do not drop supported fonts to pass the budget. Do not switch these families back to `next/font/google`: Turbopack reports a failed Google Fonts file download as `Module not found` for `@vercel/turbopack-next/internal/font/google/font` and fails the production build.
