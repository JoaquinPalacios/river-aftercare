...
Last updated: 2026-09-22 (Stripe Billing Phase 3 application code)
...
- `cancel_at_period_end` keeps entitlement `ACTIVE` until the paid period ends. `customer.subscription.deleted` sets `ENDED` and the existing 60-day `publicGuideRetentionUntil`. Public guide URLs are checked on the request. There is no cron. Drafts do not become public. `RESTRICTED` (terminal unpaid) still serves already-published guides. `PAST_DUE` keeps product access.
