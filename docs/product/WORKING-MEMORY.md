...
Last updated: 2026-09-25 (production font builds load vendored latin WOFF2 files through `next/font/local`; families, weights, CSS variables, and stored typeface ids are unchanged. Account split preparation: one SPLIT Site per preparation, `RETAIN_ON_SOURCE` for later sequential splits, split readiness separate from Practice downgrade readiness, kept Site need not be primary, destination shell uses the existing invite / legal / Checkout / `invoice.paid` path with zero Sites. No site move, guide copy, membership move, primary mutation, or Group → Practice billing. See [../architecture/ACCOUNT-SPLIT.md](../architecture/ACCOUNT-SPLIT.md). Production marketing static delivery verified on `97ce087`; homepage median TTFB 84.8ms and pricing 55.0ms from a US runner; patient and staff hosts stay `no-store`. Sentry client bundle: Turbopack `compiler.define` drops unused debug/tracing code from the shared browser chunk, 371,081 → 303,628 bytes uncompressed; early `instrumentation-client` init and server Sentry stay; production transfer of the previous chunk was about 120 KB brotli, immutable, CDN HIT)
