...
Last updated: 2026-09-20 (marketing reveal sequence consistency + staff/auth Light/Dark/System inheritance)
...
| Auth fix                   | Staff bootstrap: portal key → marketing key (same origin, usually empty on `app.`) → product cookie `aftercare-guide-ui-theme` on `.${CARE_GUIDE_ROOT_DOMAIN}` / `.localhost` → default `system`. Marketing compact/nav theme writes that cookie. Portal appearance also writes it. Patient theme does not. No Sign-in theme control added. Chromium rejects `Domain=.localhost` (public suffix), so localhost also iframes `app.` `/api/ui-theme` to set a host-only cookie plus portal localStorage. Production keeps the parent-domain cookie only. |
