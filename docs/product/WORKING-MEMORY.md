...
Capacity is enforced in `lib/clinics/site-location-capacity.ts` with the account advisory lock `clinic-site-location-capacity:${clinicId}`. Count active sites, and locations that are active on an active site. Essential is always 1/1. Practice is always 1 site, with `locationAllowance` (minimum 1). A missing entitlement is 1/1, never unlimited. Creating a site consumes one site and one location.

Group capacity has three facts. `purchasedAdditionalSiteQuantity` is the Stripe-purchased Additional Site quantity and is null until the account is explicitly configured; null is not N=0. `extraSiteAllowance` and `extraLocationAllowance` are complimentary and do not charge or refund. Usage still comes from active Clinic Site and Clinic Location rows. Stripe quantities do not create or deactivate those rows. For an ACTIVE configured Group account, `groupEffectiveAllowances` is the only formula and `groupCapacityPersistence` is the only writer of the materialized totals: `2 + N + extra sites` and `5 + N + extra locations`. A PENDING Group offer keeps its stored totals. Legacy Group rows keep stored totals until an operator saves complimentary extras. That save does not infer extras from the old total. Lowering extras below usage does not delete or deactivate sites or locations. The operator Group screen edits complimentary extras, not raw totals. Paid Additional Site bundles are not an editable field. Group Checkout is not implemented.

Approved commercial catalogue, not public marketing copy: Group base is A$449/month or A$4,490/year and includes 2 Clinic Sites and 5 Locations. Each Additional Site bundle is A$50/month or A$500/year and adds one Clinic Site and one Location. Annual billing is 12 months for the price of 10. There is no standalone paid Additional Location product, no GST, and no Stripe Tax. Public Group pricing stays custom. Practice extra locations remain the existing commercial direction (second +A$79/month, third and later +A$59/month) and are also not charged here.
...
Custom guide creation suggests `publicSlug` from the title with `suggestGuideSlug` (`lib/clinics/slug-suggestion.ts`), the same normalisation as site and location addresses. The suggestion follows the title until the user edits the slug, including clearing it. A cleared slug stays blank and fails the existing required/schema check. Editing an existing guide does not rewrite its slug when the title changes. Client suggestion does not add `-2` suffixes. `nextUnusedSlug` in `create-practice-guide.ts` remains the server collision rule for creates. Template enablement and location-copy slugs are unchanged. Cancel on the custom-guide form is a secondary link to `/guides` (`staffBtnSecondary`). It does not submit and does not use `useUnsavedChangesGuard`; that guard stays on the guide editor and practice settings. Desktop (`sm:flex-row`) shows Cancel then Create custom guide. Narrow screens (`flex-col-reverse`) show Create custom guide above Cancel, both `w-full sm:w-auto`.

Staff sidebars share `staffNavRow`. Current section uses `aria-current="page"`. Overview (`/dashboard`) and Practice (`/practice`) are exact matches, so `/practice/sites` selects Sites & Locations only. Guides, Sites & Locations, operator Clinics, operator SEO, and Billing stay current on nested routes. Account does not stay current on `/account/billing`. Hover is a neutral `color-mix` of `--staff-line` and `--staff-panel`, not the brand current treatment. Keyboard focus is `.staffNavRow:focus-visible` outline only. Sidebar links do not render `staffBtnSpinner`. Route changes keep the global top navigation progress bar. `useLinkStatus` is not added to sidebar items.

Exit support in the assisting banner is the shared `PendingSubmitButton` (`staffBtnSpinner`, label “Exit support”, pending label “Exiting…”, `aria-busy`, disabled while that form is pending). It still posts `stopOperatorClinicSupportAction`, which clears the support cookie and redirects to `/operator/clinics`. The assisting banner stays mounted until that action finishes. A failed action restores the idle control.
...
| Mapping     | Essential/Practice × monthly/yearly Price IDs from env. Unknown Price IDs fail closed. Group Price IDs were added later; this phase did not sell Group.                                                                                                                                        |
