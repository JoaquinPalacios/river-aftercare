...
Account split preparation is operator-only and does not perform that downgrade. It stores one open preparation per source Group Account and one destination shell (`Clinic` + `ClinicProfile` only). Every non-kept Site is an explicit `SPLIT`, `DEACTIVATE`, or `RETAIN_ON_SOURCE`. Exactly one Site is `SPLIT`. Retained Sites stay active on the source for a later preparation after the first is `COMPLETED`. The kept Site can be any active Site. If the current primary would be split or deactivated, the dry run previews primary promotion and does not change `isPrimary`. `READY_TO_EXECUTE` means this split could execute. `practiceDowngradeReady` is separate and is false while extra active Sites remain. The destination admin is invited through the normal Operator invitation, accepts it, accepts destination-scoped Terms and Privacy, and uses the existing offer and Checkout. `invoice.paid` is what projects billing. A shell does not need a Site for that path. V1 rejects dual membership and does not offer Group as a destination. `targetSourcePlan` may be Practice for preview. Source Stripe and the source commercial plan stay unchanged. Guide and revision map tables exist and stay empty. Public URLs and branding object keys are not rewritten. There is no execute action and no domain `COMPLETED` write. Structural writes that a future execution must observe take `clinic-account-structure:{clinicId}` inside the same transaction (`lockClinicAccountStructure` / `lockClinicAccountStructures` in `lib/entitlements/locks.ts`). An open preparation does not freeze the Account. Detail: [../architecture/ACCOUNT-SPLIT.md](../architecture/ACCOUNT-SPLIT.md).
