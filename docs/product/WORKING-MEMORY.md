...
- Local `commercialPlan` stays Practice until a trusted webhook sees the Essential Price. `scheduledCommercialPlan`, `scheduledPlanEffectiveAt`, and `stripeSubscriptionScheduleId` are the local projection. Readiness uses Phase 4 `loadEssentialDowngradeReadiness` (Essential base plus persistent extras). Conflicts do not call Stripe and do not delete anything.
- Keep Practice releases the River schedule. Cancellation already scheduled blocks a downgrade. A later portal cancellation wins: the subscription ends instead of moving to Essential. Reversing that cancellation does not put the downgrade back.
...
- Additive migration `20260923200000_add_scheduled_plan_downgrade`. Do not apply it to production from this change.
