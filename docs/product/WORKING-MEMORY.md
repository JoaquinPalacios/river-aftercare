...
| Section rhythm | `--mk-section-pad-top: clamp(8rem, 6vw, 6rem)` on inner `.band`s (top only); `--mk-chapter-pad-y: clamp(6rem, 8vw, 8rem)` on chapter starts and the closing CTA. Mobile chapter starts use `--mk-chapter-pad-y-mobile: clamp(4rem, 8vw, 8rem)`. Blends are `4rem`. Hero keeps custom spacing.  |
...
Desktop `--mk-chapter-pad-y: clamp(6rem, 8vw, 8rem)` is unchanged. First bands in `.marketingSoft` and `.marketingShowcase` use `--mk-chapter-pad-y-mobile: clamp(4rem, 8vw, 8rem)` below `47.99rem`. Closing CTA mobile `padding-top` stays `4rem`. Inner `.band` spacing is `--mk-section-pad-top` (`padding-top` only; no matching bottom pad).
