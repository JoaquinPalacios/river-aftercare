...
- A platform OPERATOR can schedule Practice → Essential for the end of the current paid period. Clinic ADMIN, STAFF, and Customer Portal cannot. Portal plan switching stays disabled.
- The same Stripe Customer and the same subscription are kept. A Subscription Schedule (SDK `22.6.2`, API `2026-08-26.dahlia`) holds the current Practice price until period end, then one Essential interval of the same length, with `proration_behavior: none` and `end_behavior: release`. No immediate invoice, credit, refund, or anchor reset.
- Local `commercialPlan` stays Practice until a trusted webhook sees the Essential Price. `scheduledCommercialPlan`, `scheduledPlanEffectiveAt`, and `stripeSubscriptionScheduleId` are the Stripe schedule projection. They are not the guide-selection state.
- Team usage above the effective Essential team allowance still blocks scheduling. Guide overage does not, once the clinic ADMIN has confirmed a keep-set that fits Essential. The operator prepares that selection locally and does not choose the guides. STAFF cannot confirm it. On `/account/billing`, an unchecked guide cannot be selected once that choice would pass the Essential custom, editable, or combined allowance. Already selected guides stay enabled. After confirmation the page shows the saved keep-set and the guides that will be retained for 60 days once Essential begins; Edit guide selection reopens the checkboxes from that saved set. The Billing page uses the same `max-w-5xl` staff width as Guides. Ordinary billing text stays in a `max-w-xl` reading column.
...
- Plan-change confirmations are one latest action. Scheduling and Keep Practice do not leave both success messages on screen. Those confirmations, and other transient action notices that use `TransientNotice`, can be dismissed locally. Dismissing one does not change the scheduled plan, Stripe, or the persistent “Scheduled change” status.
- Additive migrations `20260923200000_add_scheduled_plan_downgrade` and `20260923220000_add_downgrade_guide_selection`. Do not apply them to production from this change. The Stripe Subscription Schedule request shape is unchanged from the Sandbox acceptance.
