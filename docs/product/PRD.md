...
| Location          | `ClinicLocation` is the physical practice. The database backfills one primary account-root location per clinic. Patient URLs, portal, QR, and billing still behave as one location. Location management is not enabled. |
...
| OD-13 | Multi-location within one customer                                    | Schema foundation is in place: `Clinic` is the account and `ClinicLocation` is the physical place, with one primary account-root location backfilled per clinic. Runtime, URLs, billing, and UI remain single-location. Location management is not enabled. |
