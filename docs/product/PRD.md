...
| OD-13 | Multi-location within one customer                                    | The current single-site product reads `ClinicSite` (hostname and branding), the site’s root `ClinicLocation` (contact and emergency), and root `PracticeGuidePlacement` (public guide). `ClinicProfile` and `Clinic.slug` stay synchronized. Extra site and location creation is not enabled. Allowances are not enforced. Group pricing is provisional and not in Stripe. The multi-location product is not launched. |
