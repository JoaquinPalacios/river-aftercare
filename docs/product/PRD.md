...
| Tenant            | Architectural term for the public hostname. Today that hostname is `Clinic.slug`.                                                                                                                                               |
| Account           | `Clinic` is the paying customer. The Prisma model name stays `Clinic`.                                                                                                                                                          |
| Clinic site       | `ClinicSite` is the public identity: future subdomain and branding. One primary site is backfilled per account from `Clinic.slug` and `ClinicProfile`. Runtime still reads `Clinic.slug`.                                       |
| Location          | `ClinicLocation` is a physical practice belonging to a clinic site. One root location is backfilled per site. Patient URLs, portal, QR, and billing still behave as one location. Site and location management are not enabled. |
...
| OD-13 | Multi-location within one customer                                    | Schema foundation is `Clinic` (account) → `ClinicSite` (public identity and future subdomain) → `ClinicLocation` (physical place). One primary site and one root location are backfilled per existing account. Guides stay account-owned. Site and location allowances default to 1 and are not enforced. Group capacity is operator-configured; Group prices are not published. Runtime, URLs, billing, and UI remain single-location. |
