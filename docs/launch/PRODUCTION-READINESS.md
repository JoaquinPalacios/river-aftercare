...
| Neon Sydney PostgreSQL       | READY (wired)               | **Project exists and is used by production:** River Aftercare Production, branch `production`, region AWS Asia Pacific 2 (Sydney), PostgreSQL **18**. Pooled `DATABASE_URL` for the app; unpooled `DIRECT_URL` for `prisma migrate deploy`. Do **not** seed production. Do **not** `db push`.                                                                                                                                                                                    |
...
| DB region      | READY                                  | Neon AWS Asia Pacific 2 (Sydney). Production application is connected. Confirm backups separately.        |
...
| MUST BEFORE PRODUCTION     | Neon Sydney PG18 + backups + human-approved migrate              | Yes                   | Database is wired. Confirm PITR/backups. Schema-touching releases: merge to `main` (schema gate fails while pending) → preflight → `migrate deploy` via `DIRECT_URL` → verify → redeploy the same SHA. Never from a PR build. See [PRODUCTION-MIGRATION.md](PRODUCTION-MIGRATION.md). |
