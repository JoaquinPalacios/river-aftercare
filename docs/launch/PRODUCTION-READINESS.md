...
| Privacy / Terms    | REQUIRED BEFORE PRODUCTION             | Privacy Policy is published at `/privacy` without a draft banner. `/terms` remains a production-facing draft. **Terms legal review still required.** Operator `PRIVACY_PAGE_LEGALLY_APPROVED` is still false. |
...
| Resend             | READY  | Marketing Contact sends via the shared Resend transport using `RESEND_API_KEY`, `CONTACT_EMAIL_FROM`, and `CONTACT_EMAIL_TO`. Password-reset mail uses `AUTH_EMAIL_FROM` / optional `AUTH_EMAIL_REPLY_TO` on the same key after this change is deployed. Invitation mail is not implemented. |
...
| Password management    | READY AFTER DEPLOY                     | Change password at `/account/security`. Forgot/reset on the staff host. New passwords 12–256. Reset tokens 30 minutes, fragment URL, single-use, session invalidation. See [AUTH.md](../architecture/AUTH.md). Not live until this PR is merged and deployed.                      |
