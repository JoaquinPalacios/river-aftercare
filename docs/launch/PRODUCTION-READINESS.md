...
| Vercel (Next.js)             | READY (host) / RELEASE RISK | Production host is live (`riveraftercare.com.au`, `app.`, `*.`). Git merge to `main` still deploys **application code**. `pnpm build` generates the Prisma client and does **not** run `migrate deploy`. On `VERCEL_ENV=production` the build runs `migrate status` and fails if production is behind. Recommended dashboard change (not made from this repo): disable Production auto-deploy and promote after migrate. See [PRODUCTION-MIGRATION.md](PRODUCTION-MIGRATION.md). |
...
| Production migrations        | REQUIRED PROCESS            | Manual `pnpm prod:db:migrate -- --apply` (or the equivalent `env -u DATABASE_URL -u DIRECT_URL DOTENV_CONFIG_PATH=.env.neon-production` `prisma migrate deploy`). Never automatic on PR, Preview, or ordinary Vercel build. `20260919140000_add_clinic_typeface` was applied after a code-before-schema P2022 incident. Follow migrate-before-promote.                                                                                                                           |
...
- Vercel → Next.js
- Neon Sydney → PostgreSQL 18 (wired; migrate with `DIRECT_URL` before promoting code)
...
| MUST BEFORE PRODUCTION     | Neon Sydney PG18 + backups + migrate-before-promote              | Yes                   | Database is wired. Confirm PITR/backups. Every schema-touching release: preflight → `migrate deploy` via `DIRECT_URL` → verify → then promote code. Never from a PR build. See [PRODUCTION-MIGRATION.md](PRODUCTION-MIGRATION.md). |
