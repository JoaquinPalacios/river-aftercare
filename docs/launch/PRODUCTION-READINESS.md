...
| Operator invitations   | READY AFTER DEPLOY                     | Operator Team at `/operator/clinics/[clinicId]/team`. Invite → pending User, no membership → email fragment link → recipient sets password → membership. Successful delivery redirects to Team. Operators can remove access (keep User/password, kill sessions) and restore passworded zero-membership Users without a new invitation. Clinic admins cannot invite. No new Prisma migration. |
