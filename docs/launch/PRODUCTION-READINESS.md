...
| Password management    | READY AFTER DEPLOY                     | Change password at `/account/security`. Forgot/reset on the staff host. New passwords 12–256. Reset tokens 30 minutes, fragment URL, single-use, session invalidation. See [AUTH.md](../architecture/AUTH.md).                                                                                                                                                                                                                                   |
| Operator invitations   | READY AFTER DEPLOY                     | Operator Team at `/operator/clinics/[clinicId]/team`. Invite → pending User, no membership → email fragment link → recipient sets password → membership. Successful delivery redirects to Team. Operators can change active ADMIN ↔ STAFF roles, remove access (keep User/password, kill sessions), and restore passworded zero-membership Users without a new invitation. Clinic admins cannot invite or change roles. No new Prisma migration. |
...
| Backup restore   | PARTIAL                                | Isolated historical child-branch recovery proven. Do not rewind production as a test. See [NEON-RECOVERY.md](NEON-RECOVERY.md). |
