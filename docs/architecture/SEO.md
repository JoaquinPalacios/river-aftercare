...
| Marketing `/privacy`, `/terms`                                              | apex / `localhost`              | **no** until approval | yes    | yes     | `/privacy` public copy is published without a draft banner; `/terms` remains an explicit legal draft. Page-level `noindex, follow` until counsel/operator approval. Still listed in sitemap and `llms.txt`. |
