...
| Icon                   | River pack: `public/favicons/*` plus Next.js `app/favicon.ico`, `app/icon.png`, `app/apple-icon.png`. Patient tenant guides additionally emit a clinic `faviconUrl` via tenant metadata when configured; marketing, staff, login, and operator stay on the River pack. Next.js still prepends the root `favicon.ico` file-convention link on every document. |
