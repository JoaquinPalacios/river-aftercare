...
| Icon                   | River pack: `public/favicons/*` plus Next.js `app/favicon.ico`, `app/icon.png`, `app/apple-icon.png`. Patient tenant guides may additionally emit a clinic `faviconUrl` via tenant metadata; marketing, staff, login, and operator stay on the River pack. |
