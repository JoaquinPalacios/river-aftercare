...
| Icon                   | River pack from `public/favicons/*` via `PRODUCT_HEAD_METADATA` on marketing, staff, and aftercare root layouts. There is no root `app/favicon.ico` / `app/icon.png` / `app/apple-icon.png` file convention. Patient tenant guides override `icons` with the clinic `faviconUrl` when configured (PNG `rel="icon"` + `apple-touch-icon` only). Missing clinic favicon keeps the River pack. Tenant HTML may still include the River `/favicons/site.webmanifest`; that is not a tab-icon competitor. |
