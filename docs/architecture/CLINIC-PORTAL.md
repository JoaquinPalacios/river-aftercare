...
Desktop: a slim sticky application toolbar (about 56–64px) holds the guide title, status pill, quiet save-state (`Saved` / `Unsaved changes` / `Saving…`), and Cancel / Save draft / the publication action. A draft or unpublished guide offers **Publish guide**. A published guide with nothing new to publish offers **Unpublish**, which uses the existing unpublish confirmation. Draft changes on a published guide keep **Publish guide** and also offer **Unpublish**. The right column is the live patient timeline preview, sticky, with viewport-based max-height. Status and actions no longer live in a large preview-rail card.
...
- A Practice → Essential downgrade is self-service for the clinic ADMIN on `/account/billing`. River checks team and guide readiness, the administrator chooses the keep-set when clinic-owned usage is above Essential, and Schedule downgrade applies at the next renewal. Keep Practice before that date releases the schedule. Cancel plan change is also available before the downgrade is scheduled, including after a partial Stripe attempt. River reads the live subscription and releases a schedule that belongs to the current attempt before it deletes the keep-set. A failed cancellation leaves that choice in place. STAFF can see the status and cannot change the plan. The operator clinic page shows the same state, including an open attempt, and does not approve the change. Guides that are not kept become read-only for 60 days after the Essential price actually applies. They leave active capacity and the public clinic index. A guide that was already published keeps its direct patient URL until that deadline, and the retained-guide card shows that live URL and the expiry date. Drafts and unpublished guides stay private and are not described as live. Restore during the 60 days needs a free place on the current plan, including any operator-granted allowance. Expired rows are not purged by a job. See [BILLING.md](BILLING.md).
