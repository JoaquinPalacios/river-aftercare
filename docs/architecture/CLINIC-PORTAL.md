...
If a published tenant page fails, patients see generic River Aftercare copy: the aftercare guide cannot load right now, please try again shortly. The error boundary does **not** query `ClinicProfile` or Neon to recover branding. When the tenant layout already rendered, clinic chrome from that layout may remain around the fallback. Unknown-tenant 404s stay practice-neutral. Staff/operator links are never included. Do not invent clinical or emergency instructions in these fallbacks.
