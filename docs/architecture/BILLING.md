...
- Cancellation scheduled (`cancel_at_period_end`) keeps entitlement `ACTIVE` through `paidThrough`. Removing it returns billing status to `ACTIVE`. `customer.subscription.deleted` sets `ENDED`, `subscriptionEndedAt`, and `publicGuideRetentionUntil` (existing 60-day rule).
...
- Operator-assisted Essential → Practice updates the existing subscription item. Proration is `always_invoice`. Payment behavior is `pending_if_incomplete`, so the Practice price is not current until Stripe accepts payment. The local plan changes from the webhook, not from the button. Practice → Essential is deferred until guide and team limits exist. Monthly ↔ annual is not offered.
