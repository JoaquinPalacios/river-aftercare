...
Payload policy: `sendDefaultPii: false`, `includeServerName: false`, `tracesSampleRate: 0`, `maxBreadcrumbs: 0`, no local variables, no attachments, no module inventory. Default integrations that collect console, request data, machine context, or database/performance traces are filtered out. `beforeSend` strips `user`, `server_name`, `modules`, breadcrumbs, extra, request headers/cookies/query/body, and device/OS/culture/app-memory contexts. URLs keep scheme/host/path only. Controlled tags are `component`, `failure_code`, and `environment`. Exception type, redacted message, stack filenames, and line/column numbers are kept.
