...
Structured events only: `password_changed`, `password_reset_requested`, `password_reset_email_failed`, `password_reset_completed`, `profile_name_changed`, `email_change_requested`, `email_change_completed`, `email_change_email_failed`, `invitation_created`, `invitation_email_failed`, `invitation_resent`, `invitation_cancelled`, `invitation_accepted`, `clinic_access_removed`, `clinic_access_restored`, `clinic_role_updated`, `clinic_membership_deactivated`, `clinic_membership_reactivated`, `operator_clinic_settings_updated`, `operator_allowance_extra_updated`. Prefer user id (and clinic id for invitations / membership / operator clinic edits). The allowance-extra event records the operator user id, clinic id, dimension (`team_members`, `custom_guides`, or `template_adaptations`), previous extra, new extra, and effective allowance. Never log passwords, raw tokens, token hashes, reset/invite URLs, session tokens, API keys, mail bodies, or invitee names and emails. Unknown emails are not logged.
...
Security logging: `invitation_created`, `invitation_email_failed`, `invitation_resent`, `invitation_cancelled`, `invitation_accepted`, `clinic_access_removed`, `clinic_access_restored`, `clinic_role_updated`, `clinic_membership_deactivated`, `clinic_membership_reactivated`, `operator_allowance_extra_updated`. User id + clinic id (plus actor user id for status changes). The allowance-extra event records operator user id, clinic id, dimension, previous extra, new extra, and effective allowance, without invitee email or name. Never passwords, raw tokens, hashes, URLs, or provider errors.
