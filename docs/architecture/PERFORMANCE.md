...
- **Application tenant 404:** unknown tenant, unknown/draft/disabled guide, or pinned draft revision render `app/(aftercare)/not-found.tsx` (River Aftercare logo, “This aftercare guide isn’t available”, link to `https://riveraftercare.com.au`). Same copy for missing, removed, unpublished, or never-existed slugs. Known-tenant layout may still apply CSS variables around that page; the 404 covers clinic chrome so it does not advertise another tenant.
