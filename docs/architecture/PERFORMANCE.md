...
8. Custom fonts and other third-party assets require an explicit performance review. Clinic typefaces are a curated allowlist in `lib/branding/font-files`, loaded with `next/font/local` from the patient tenant layout with `preload: false`. Marketing and staff keep Geist the same way. Builds and browsers do not request `fonts.googleapis.com` or `fonts.gstatic.com`.
