...
function describeMismatch(existing) {
  ...
  if (!existing.isSample) {
    return "Sample designation is missing or unsafe (isSample must be true).";
  }
...
export function formatDemoExtractionBootstrapPlan(plan) {
  ...
  if (plan.action === "create") {
    lines.push(
      "Would create:",
      `- 1 GuideTemplate slug=${plan.template.slug} title=${plan.template.title} specialty=${plan.template.specialty} isActive=true isSample=true`,
