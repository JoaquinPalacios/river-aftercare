...
export function formatPrismaReleaseReport(evaluation) {
  ...
  const lines = [
    "========== PRISMA RELEASE GATE ==========",
    "This change includes Prisma schema and/or migration files.",
    "Production must migrate-before-promote. Vercel does not apply migrations.",
    "",
    ...
    "  5. Only then promote application code that requires the new schema.",
    "",
  ...
  if (evaluation.errors.length === 0) {
    lines.push(
      "Pairing check passed. This is still a schema-touching release — do not deploy application code before production migrate deploy."
    );
  }
